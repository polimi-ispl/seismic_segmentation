...
def mid_intensity_high_texture(in_content):
    ...
    score = mean_std_weight * ch_mean_score + \
        (1 - mean_std_weight) * ch_std_score
    return score
...
def patch_array_shape(in_size, patch_size, patch_stride):
    win_indices_shape = (
        (np.array(in_size) - np.array(patch_size)) // np.array(patch_stride)) + 1
...
class PatchExtractor:
    ...
    def __init__(self, dim, offset=None, stride=None, rand=None, function=None, threshold=None,
        ...
        if rand is not None and function is not None:
            raise ValueError(
                'rand and function cannot be set at the same time')
    ...
    def extract(self, in_content):
        ...
        if in_content.ndim != self.ndim:
            raise ValueError(
                'in_content shape must a tuple of length {:d}'.format(self.ndim))
        ...
        for dim_idx, dim_offset in enumerate(self.offset):
            ...
            in_content = in_content.take(
                range(dim_offset, dim_max), axis=dim_idx)
        ...
        if self.dim == self.stride:
            ...
            for dim_idx in range(self.ndim):
                dim_max = (in_content.shape[dim_idx] //
                           self.dim[dim_idx]) * self.dim[dim_idx]
                in_content_crop = in_content_crop.take(
                    range(0, dim_max), axis=dim_idx)
        ...
        if self.rand:
            ...
            if self.function_handler is not None:
                ...
                patch_scores = np.asarray(
                    list(map(self.function_handler, patch_array)))
    ...
    def reconstruct(self, patch_array):
        ...
        image_shape_computed = tuple(
            (np.array(patch_idx) - 1) * np.array(patch_stride) + np.array(patch_shape))
                    ...
                    if ndim > 2:
                        for j in np.arange(0, image_shape[2] - patch_shape[2] + 1, patch_stride[2]):
                            if ndim > 3:
                                ...
                                norm_mask[h:h + patch_shape[0], i:i +
                                          patch_shape[1], j:j + patch_shape[2]] += 1
                                counter += 1
                    else:
                        image_recon[h:h + patch_shape[0], i:i + patch_shape[1]
                                    ] += patch_array_unwrapped[counter, :, :]
                        norm_mask[h:h + patch_shape[0],
                                  i:i + patch_shape[1]] += 1
                        counter += 1
            else:
                image_recon[h:h + patch_shape[0]
                            ] += patch_array_unwrapped[counter, :]
